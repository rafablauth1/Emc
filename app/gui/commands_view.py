...
from app.instruments import agilent_53131a_commands, chroma_commands, ucs500n_commands
...
COMMAND_FIELDS = {
    ...
    "chroma": [
        ...
        ("OUTPUT_RELAY", "Relé de saída (use {state})"),
        ("OUTPUT_MODE", "Modo de saída (use {mode})"),
        ("VOLTAGE_RANGE", "Faixa de tensão (use {range})"),
        ("SET_VOLTAGE_AC", "Tensão AC (use {voltage})"),
        ("SET_FREQUENCY", "Frequência (use {frequency_hz})"),
        ("SET_PULSE_VOLTAGE_AC", "Pulso — tensão AC (use {voltage})"),
        ("SET_PULSE_PERIOD_MS", "Pulso — período ms (use {period_ms})"),
        ("SET_PULSE_DUTY_PCT", "Pulso — duty cycle % (use {duty_pct})"),
        ("SET_PULSE_START_PHASE", "Pulso — fase inicial (use {phase_deg})"),
        ("SET_PULSE_COUNT", "Pulso — contagem (use {count})"),
        ("TRIGGER_STATE", "Trigger ON/OFF (use {state})"),
        ("TRIGGER_STATE_QUERY", "Consultar estado do trigger"),
